...
Route::middleware(['install'])->group(function () use ($email_verification, $allow_signup) {
    ...
    Route::group(['middleware' => $email_verification == 'enabled' ? ['auth', 'verified'] : ['auth']], function () {
        ...
        Route::group(['middleware' => ['permission'], 'prefix' => 'admin'], function () {
            ...
            //Next of Kin Details Controller      
            Route::resource('next_of_kin', 'NextOfKinController');  
            
            //Next of Guarantor Controller      
            Route::resource('guarantors', 'GuarantorController');   

            //Next of Banking Details Controller      
            Route::resource('banking_details', 'BankingDetailController');   
            ...
            Route::post('registers/get_register_table_data', 'RegistersController@get_register_table_data');
            Route::get('registers', 'RegistersController@index')->name('registers.index');
            Route::get('registers/view/{name?}', 'RegistersController@view')->name('registers.view');
            ...
            Route::match(['get', 'post'], 'loans/repay/{id}', 'LoanController@repay')->name('loans.repay');
            // Route::get('loans/print-receipt/{id}', 'LoanController@receipt')->name('loans.print-receipt');
