...
if (!function_exists('create_option')) {
    function create_option($table, $value, $display, $selected = '', $where = NULL) {
        ...
        $query = DB::select("SELECT * FROM $table $condition");// AND WHERE deleted_at IS NULL
        foreach ($query as $d) {
...
if (!function_exists('transaction_status')) {
    function transaction_status($status) {      
        return match($status){
            0,"0"=> "<span class='badge badge-danger'>" . _lang('Cancelled') . "</span>",
            1,"1" => "<span class='badge badge-warning'>" . _lang('Pending') . "</span>",
            2,"2" => "<span class='badge badge-success'>" . _lang('Completed') . "</span>",
            default => "<span class='badge badge-danger'>" . _lang('Invalid') . "</span>",
        };
    }
}
